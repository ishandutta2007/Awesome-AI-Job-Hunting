...
class ApplyRecordsApplication(unittest.TestCase):
    ...
    def test_tracker_header_matches_outcome(self):
        ...
        for name, text in (
            ("outcome.md", OUTCOME.read_text(encoding="utf-8")),
            ...
            header = next(
                (
                    ...
                    if ln.strip().startswith("date,company,")
                ),
                None,
            )
            self.assertEqual(
...
class DraftedMeansDraftedToEveryReader(unittest.TestCase):
    ...
    CASES = [
        ...
        (
            GMAIL_SYNC,
            "### Step 7a",
            "also set `date` to the email's date",
        ...
        (
            NOTION_SYNC,
            None,
            "omit when the status is `drafted`",
